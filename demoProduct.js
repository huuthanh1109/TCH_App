...
export default function Products({navigation}) {
  ...
  useEffect(() => {
     ...
     const getApiProduct1 = async () =>{
      const rs = await getProduct1()
        // var arrProductPortfolio = rs.data;
        // var valueId = arrProductPortfolio.filter(e => e.id)
        // console.log(valueId)
        // return valueId
        setProduct1(rs.data)
    }
    ...
    const getApiProduct = async () =>{
      ...
      var loc =rs.data.data
      var a = loc.filter(categId =>categId.categ_id.find(e => e===10))
        console.log(a)
      setProduct(product.concat(a))
  ...
  console.log('hello',product1)
  // const arr = product1.filter((e) => e.product.filter( f => f.product_name ==="Latte Đá"))
  ...
  const renderItem = ({ item }) => (
    ...
    <Pressable style={{ width: '100%', flexDirection:'row',justifyContent:'space-between',backgroundColor:'#fff',
          ...
          <View style={styles.wap}>
            ...
            <View style={styles.wapHeading}>
              ...
              <View>
                ...
                <Text style={styles.textDesr}>Các món sẽ được giao đến địa chỉ của bạn</Text>
              </View>
            </View>
            

            <View style={styles.wapScrollList}>
              <Pressable style={styles.listItem} onPress={() => panelRef.current.togglePanel()}>
